...
export default function Csdn() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await fetch('/api/csdn')
          ...
          return (
            <li key={index} className={clsx(CsdnStyle['csdn-item'])}>
              <div className="border rounded-2xl p-4 w-full shadow cursor-default hover:bg-gray-50">
                <a className="text-base text-gray-800 font-bold break-words text-left tooltip tooltip-bottom hover:underline" data-tip={dayjs(item.pubDate).format('YYYY-MM-DD HH:mm:ss')} href={item.link} target="_blank" rel="noopener noreferrer">{item.title}</a>
